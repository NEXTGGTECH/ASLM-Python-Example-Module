...
def save_host_theme_payload(data: dict[str, Any]) -> None:
    ...
    _atomic_write_json(HOST_THEME_FILE, data)


# Return Settings/host_theme.json or None if missing or invalid.
def load_host_theme() -> dict[str, Any] | None:
    ...
    if not HOST_THEME_FILE.exists():
        return None
    try:
        raw = HOST_THEME_FILE.read_text(encoding="utf-8")
...
def get_effective_theme() -> str:
    ...
    if payload:
        return str(payload.get("theme", "dark")).lower()
    return "dark"
